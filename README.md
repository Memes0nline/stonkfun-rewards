...
The first scan covers the **last 7 days** and usually takes a few minutes. After that there are two buttons. **Check latest data** checks from your last scan up to now. **Scan more** lists your history back to **2026-08-01** (StonkFun launched in early August 2026) in batches of up to 7 days, newest first: **Load** on a batch not loaded yet reads every batch from your oldest loaded day back to it, one after another, and stopping keeps every batch already finished. Each button shows the UTC dates it will read, and the scan window shows the batch and the day it is reading. Every day a scan reads is checked against a second list of its transactions from Helius, and anything the first read missed is fetched. A batch loaded before that check existed offers **Check** in Scan more, which reads it again and adds anything missed, without counting a saved payout twice; once checked it reads **Confirmed**.
